	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::CollapsingHeader("Point Light Lighting Settings"))
			{
				...
				if (ImGui::DragFloat("Light Mesh", &(illuminationBuffer->getPointRef().volume), 0.1f, 0.1f, 5.0f))
				{
				}
			}
		...
		GameObject lightOBJ = scene->CreateEntity("LightOBJ");
		{
			...
			lightOBJ.emplace<RendererComponent>().SetMesh(vao).SetMaterial(grassMat);
			lightOBJ.get<Transform>().SetLocalScale(0.5f,0.5f,0.5f);
		} 
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			if (drawLightStuff) {
				//lightOBJ.get<RendererComponent>().SetMesh(LIGHTMONKEY);
			}
			else {
				//lightOBJ.get<RendererComponent>().SetMesh(LIGHTSPHERE);
			}
